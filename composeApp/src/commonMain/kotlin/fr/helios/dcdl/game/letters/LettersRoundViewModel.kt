...
class LettersRoundViewModel: ViewModel() {
    ...
    fun onClickInitialTile(position: Int) {
        ...
        val newAnswer = answer + tileUI.id
    ...
    fun getOnLetterDragStart(tileId: Int, getLetterOffset: (Int) -> Offset): (Offset) -> Unit = { cursorOffset ->
        _uiState.value = _uiState.value.copy(cursorOffsetInDraggingLetter = cursorOffset)
    ...
    fun getOnLetterDrag(bounds: Rect): (PointerInputChange, Offset, density: Density) -> Unit = { change, offset, density ->
        change.consume()
        val uiState = _uiState.value
        uiState.draggingLetter?.let { draggingLetter ->

            updateDraggingLetter(
                draggingLetterOffset = uiState.draggingLetterOffset + offset
            )

            val cursorOffset = uiState.draggingLetterOffset + uiState.cursorOffsetInDraggingLetter
            if (bounds.contains(cursorOffset)) {
                val indexToAdd = LettersUi.calculateDropIndex(
                    density = density,
                    position = cursorOffset,
                    box = bounds,
                    currentSize = uiState.answer.size
                )
...
data class LettersRoundViewModelUiState(
    ...
    val cursorOffsetInDraggingLetter: Offset = Offset.Zero
) {
